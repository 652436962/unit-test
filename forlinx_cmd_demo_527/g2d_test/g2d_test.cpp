...
#elif defined(_T113_)
    G2D_SCALE, G2D_DECOMPOSE, G2D_COMPOSE, G2D_ROTATE, G2D_HFLIP, G2D_VFLIP, G2D_FORMAT_CONVERT_RGBA2YUV_ByFd,
    G2D_FORMAT_CONVERT_YUV2RGBA_ByFd, G2D_FORMAT_CONVERT_YUV2RGBA_ByPhy,
#else
    G2D_MODE_NULL,
#endif
};
...
//  scale
char *pScaleRgbaInPath = "/tmp/g2d_test/1024x600.rgba";//g2d_test 0
...
// flips v
char *pFlipsRgbaVOutPath = "/tmp/g2d_test/1024x600_v_flips.rgba";
...
{
    printf("================Usage================\n");
    printf("g2d_test 0	  means: 1024x600*rgba scale to 1280x720*rgba\n");
    ...
    printf("g2d_test 4	  means: 1024x600 rgba vertical flips\n");
...
int main(int argc, char *argv[])
{
    ...
    switch (testid) {
        ...
        case G2D_HFLIP: {
            ...
            gettimeofday(&t2, NULL);
            ALOGD("g2d h flips ret:%d, use time:%lld us", ret,
                  int64_t(t2.tv_sec) * 1000000LL + int64_t(t2.tv_usec) - (int64_t(t1.tv_sec) * 1000000LL + int64_t(t1.tv_usec)));
        ...
        case G2D_VFLIP: {
            ...
            WritePicFileContent(pFlipsRgbaVOutPath, &m_DispMemOps, outPicSize);
