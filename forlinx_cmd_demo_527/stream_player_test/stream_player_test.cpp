...
int find264NALFragment(char* data, size_t size, int* fragSize)
{
    ...
    static const char kStartCodeLegecy[3] = { 0x00, 0x00, 0x01 };
    if (size < 4) {
    ...
    if (!memcmp(kStartCode, data, 4)) {
        offset = 4;
    }else if(!memcmp(kStartCodeLegecy, data, 3)){
        offset = 3;
    }else{
        ALOGD("StartCode not found in %.2x %.2x %.2x %.2x", data[0], data[1], data[2], data[3]);
        return -2;
    }
    while (offset + 3 < size && memcmp(kStartCode, &data[offset], 4)  && memcmp(kStartCodeLegecy, &data[offset], 3)) {
        ++offset;
    }
    if (offset > (size - 4)  || offset > (size - 3)) {
        ALOGD("offset>(size-4)");
        offset = size;
    }
    *fragSize = offset;
...
int h264Test()
{
    ...
    if(strlen(testParam.intputFile) == 0)
    {
...
int h265Test()
{
    ...
    decodeParam.dispW = 1024; //600;
    decodeParam.dispH = 600; //351;
    decodeParam.rotation = Angle_0;
    decodeParam.tvOut = NONE;
    if(strlen(testParam.intputFile) == 0)
    {
...
//    getExpectSize(&dstW,&dstH);
    ...
    if(strlen(testParam.intputFile) == 0)
    {
       strcpy((char*)testParam.intputFile, INPUT_JPG);
    }
    ...
    for (int i = 1; i <= fileCount; i++) {
        ...
        snprintf(inFilePath, sizeof(inFilePath), "%s%d.jpg", testParam.intputFile, i);
