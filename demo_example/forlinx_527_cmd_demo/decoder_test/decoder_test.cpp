...
void* decodeThread(void* arg)
{
    ...
    while (decodePos < fileSize) {
        ...
        if ((0 < ret) && (ret <= outputLen)) {
            if (outputFile == NULL) {
                snprintf(filename, sizeof(filename), "%s-%d.yuv", testParam.outputFile, parm->pixelFormat);
    ...
    printfArgs(*parm);
    ALOGD("output file is saved:%s", filename);
...
int  multiDecoderTest()
{
    ...
    int startFmt = PIXEL_YV12;
    int stopFmt = PIXEL_NV21;
